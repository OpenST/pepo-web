    ...
    let videoDetails = apiData.video_details &&  apiData.video_details[userVideoId];
      if (videoDetails){
        ...
        if (Array.isArray(videoDetails.link_ids) && videoDetails.link_ids.length > 0){
          ...
          if(apiData.links && apiData.links[linkId]){
            resultObj['description_link'] = apiData.links[linkId].url;
            resultObj['description_display_link'] = apiData.links[linkId].url.replace(/^(?:https?:\/\/)?(?:www\.)?/i, '').replace(/\/$/, '');
          }
        }
