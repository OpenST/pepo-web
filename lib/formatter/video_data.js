...
function videoDataFormatter(apiData){
  ...
  let videoUrlsObj = _getVideoSharableData(apiData);
  Object.assign(resultObj, videoUrlsObj);

  return Object.assign(_getDefaultResultObj() ,resultObj);
}

function _getVideoSharableData(apiData) {
  let sharableObject = {};
...
function _getPosterImageUrl(videoData, apiData){
  ...
  if (posterImageId && apiData.images &&  apiData.images[posterImageId] ) {
    ...
    //   posterObject['mobile_video_poster_url'] = posterImageData.resolutions['288w'].url;
    // } else
    ...
    // if (posterImageData && posterImageData.resolutions && posterImageData.resolutions['288w']) {
    //   posterObject['web_video_poster_url'] = posterImageData.resolutions['288w'].url;
    // } else
      if (posterImageData && posterImageData.resolutions && posterImageData.resolutions.original){
...
function _getVideoUrls(videoData) {
  ...
  } else if (videoData && videoData.resolutions && videoData.resolutions.original){
