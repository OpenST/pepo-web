...
class Base {
  ...
  constructor(cookies, headers) {
    const oThis = this;
    oThis.cookies = cookies;
    oThis.headers = headers;

    oThis.serviceBaseRoute = null;
    oThis.apiUrl = coreConstants.PEPO_API_DOMAIN;
  }
  ...
  _setCookieInHeader() {
    ...
    for (let key in oThis.cookies) {
      let val = oThis.cookies[key];
      cookie = cookie + `${key}=${encodeURIComponent(val)}; `;
    }
  ...
  _parseCookieFromResponse(response) {
    ...
    oThis.cookies[cookieConstants.newCookieName] =  newCookies;
  }

  async _parseResponse(response) {
    const oThis = this;

    let jsonResp = JSON.parse(response.responseData);
