...
class Base {
  ...
  constructor(headers) {
    ...
    oThis.apiUrl = coreConstants.PEPO_DOMAIN;
  }
  ...
  async _send(requestType, endpoint, queryParams = {}) {
    ...
    let resource = oThis.apiUrl + '/api' + oThis.serviceBaseRoute + endpoint;
  ...
  async _parseResponse(response) {
    ...
    switch (jsonResp.err.code) {
      ...
      case 'INTERNAL_SERVER_ERROR': {
        console.log('==jsonResp.err.internal_id======', jsonResp.err.internal_id);
        if (jsonResp.err.internal_id === apiInternalCodesConstants.alreadyRegisteredUserInApp) {
          finalResp = responseHelper.error({
            internal_error_identifier: jsonResp.err.internal_id,
